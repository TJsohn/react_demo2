    ...
    return (
        <div className="bookCard">
            <div className="bookCard-header">
                ...
                onClick={onToggleStock}>{inStock ? "In stock" : "Out of Stock"}</p>
                <h2>{title}</h2>
            ...
            <div className="bookCard-content">
                {isEditing ? (<input type="number" value={newPrice} onChange={(e) => setNewPrice(e.target.value)} /> ): (<p className="price">{price}€</p>)}
            </div>
            <div className="bookCard-footer">
                <Link className="secondary" to={`/books/${id}`}>See more</Link>
